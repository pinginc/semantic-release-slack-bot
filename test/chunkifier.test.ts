...
describe('test chunkifyArray', () => {
  ...
  it('should return only one chunk if message is <= maxLength', () => {
    ...
    const actual = chunkifyArray(expected, (expected[0] + DEFAULT_DELIMITER).length);
    assert.deepStrictEqual(actual, expected);
  });

  it('should capture last chunk', () => {
    const messageLines = 'hello world\nhere we come\nto rule'.split(DEFAULT_DELIMITER);
...
describe('test chunkifyString', () => {
  ...
  it('should return only one chunk if message is <= maxLength', () => {
    ...
    const actual = chunkifyString(expected, (expected + DEFAULT_DELIMITER).length);
    assert.deepStrictEqual(actual, [expected]);
  });
