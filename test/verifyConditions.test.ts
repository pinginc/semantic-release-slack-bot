...
describe('test verifyConditions', () => {
  ...
  describe('test slack webhooks options', () => {
    ...
    it('should throw if neither slackWebhook and environment variable SLACK_WEBHOOK are set', () => {
      assert.throws(
        ...
        new SemanticReleaseError('No Slack web-hook defined.', 'ENOSLACKHOOK', getMissingWebhookError('SLACK_WEBHOOK'))
      );
    });

    it('should throw if slackWebhook is not set and slackWebhookEnVar give an empty environment variable', () => {
      assert.throws(
        () => verifyConditions({ ...defaultPluginConfig, slackWebhookEnVar: 'CUSTOM_WEBHOOK' }, fakeContext),
        new SemanticReleaseError('No Slack web-hook defined.', 'ENOSLACKHOOK', getMissingWebhookError('CUSTOM_WEBHOOK'))
      );
    });

    it('should not throw if slackWebhook is provided', () => {
      verifyConditions({ ...defaultPluginConfig, slackWebhook: 'MY SLACK WEBHOOK' }, fakeContext);
    });
  ...
  describe('test slack token and channel options', () => {
    ...
    it('should throw if neither slackChannel and environment variable SLACK_CHANNEL are set but slackToken is', () => {
      assert.throws(
        () => verifyConditions({ ...defaultPluginConfig, slackToken: 'some token' }, fakeContext),
  ...
  describe('test branchesConfig options', () => {
    ...
    it('should throw if branchesConfig is not an array', () => {
      assert.throws(
        () => verifyConditions({ ...defaultPluginConfig, branchesConfig: {} }, fakeContext),
        new SemanticReleaseError(
